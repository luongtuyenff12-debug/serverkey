...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if(pathname === '/' && req.method === 'GET'){
      /* ---- Forward sang /api/verify nếu DEX gọi root / kèm param "key" ----
         DEX trong classes9.dex lưu URL không có path "/api/verify", chỉ có
         "https://serverkey-u8w6.onrender.com" → khi gọi sẽ hit endpoint này.
         Giải pháp: nếu request có query param "key" thì xử lý như /api/verify
         thay vì trả về trang HTML. Toàn bộ logic bên dưới là bản sao y chang
         của /api/verify — KHÔNG thay đổi bất kỳ logic nào. */
      if(url.searchParams.get('key')){
        pathname = '/api/verify';
        // fall-through xuống block /api/verify bên dưới
      } else {
        res.writeHead(200, {
