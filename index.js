...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if((pathname === '/api/loadlib/check' || pathname === '/api/loadlib/check-cakmods')
      ...
      // Proxy sang server gốc (check.php) để lấy thông tin phiên bản/URL tải
      const checkTarget = LOADLIB_CONFIG.CHECK_URL
        + (version ? `?version=${encodeURIComponent(version)}&lib=${encodeURIComponent(targetLib)}`
                   : `?lib=${encodeURIComponent(targetLib)}`);

      // Thực hiện request tới upstream (dùng https native của Node)
      let upstreamData;
      try {
        upstreamData = await new Promise((resolve, reject) => {
          const reqOpts = new URL(checkTarget);
          const proto   = reqOpts.protocol === 'https:' ? require('https') : require('http');
          const options  = {
          ...
          const upReq = proto.request(options, upRes => {
            ...
            upRes.on('end', () => {
              if(upRes.statusCode !== 200){
                return reject(new Error(`upstream_http_${upRes.statusCode}`));
              }
              try { resolve(JSON.parse(raw)); }
              catch(e){ reject(new Error('upstream_json_parse_error')); }
            });
          });
          upReq.on('error',   reject);
          upReq.on('timeout', () => { upReq.destroy(); reject(new Error('upstream_timeout')); });
          upReq.end();
        });
      } catch(upErr){
        const code = String(upErr && upErr.message || upErr);
        const httpCode = code.match(/upstream_http_(\d+)/)?.[1];
        console.error(`[LoadLib] Lỗi gọi upstream check.php từ ${loadlibIp}: ${code}`);
        return sendJSON(res, 502, {
          status : 'ERROR',
          error  : `Không thể kết nối server thư viện: ${code}`,
          message: httpCode
            ? `Server returned error: ${httpCode}`
            : 'Server returned error: 502'
        });
      }

      // Validate response từ upstream — đảm bảo đủ trường [B] nếu lỗi
      if(!upstreamData || upstreamData.status !== 'SUCCESS'){
        const errStatus  = (upstreamData && upstreamData.status)  || 'UNKNOWN_ERROR';
        const errDetail  = (upstreamData && upstreamData.error)   || 'Upstream trả về lỗi không xác định.';
        const errMessage = (upstreamData && upstreamData.message) || 'Server returned error: unknown';
        return sendJSON(res, 200, {
          status : errStatus,
          error  : errDetail,
          message: errMessage
      ...
      // Xác thực SHA256 certificate từ upstream trùng với giá trị đã cấu hình
      const remoteSha = String(upstreamData.sha256 || '').toLowerCase().trim();
      const allowedSha= LOADLIB_CONFIG.ALLOWED_CERT_SHA256.toLowerCase();
      if(remoteSha && remoteSha !== allowedSha){
        console.warn(`[LoadLib] SHA256 KHÔNG KHỚP từ ${loadlibIp}: remote=${remoteSha}`);
        return sendJSON(res, 200, {
          status : 'SHA256_MISMATCH',
          error  : 'Chứng chỉ thư viện không hợp lệ. Liên hệ admin.',
          message: 'Server returned error: sha256_mismatch'
        });
      }

      // Lấy fileSize thực bằng HEAD request để AutoLoader.java có thể kiểm tra kích thước file
      let fileSize = upstreamData.fileSize || upstreamData.file_size || 0;
      if(!fileSize && upstreamData.file_url){
        try{
          fileSize = await new Promise((resolve) => {
            const fUrl = new URL(upstreamData.file_url);
            const proto = fUrl.protocol === 'https:' ? require('https') : require('http');
            const headReq = proto.request({
              ...
              resolve(Number.isFinite(cl) && cl > 0 ? cl : 0);
            });
            headReq.on('error',   () => resolve(0));
            headReq.on('timeout', () => { headReq.destroy(); resolve(0); });
            headReq.end();
          });
        }catch(e){ fileSize = 0; }
      }

      // Thành công — trả về cấu trúc tương thích CẢ HAI:
      //   [A] AutoLoader.java fetchServerConfig(): fileUrl, fileSize, sha256, version
      //   [B] Dashboard admin (proxy): file_url, sha256, version, message
      const fileUrl = upstreamData.file_url || upstreamData.fileUrl || '';
      console.log(`[LoadLib] OK — ${loadlibIp} => ${fileUrl} v${upstreamData.version || 'N/A'} size=${fileSize}`);
      return sendJSON(res, 200, {
        status  : 'SUCCESS',
        version : upstreamData.version || '',
        fileUrl : fileUrl,         // ← AutoLoader.java fetchServerConfig() dùng field này
        file_url: fileUrl,         // ← Dashboard admin/proxy dùng field này
        sha256  : upstreamData.sha256 || '',
        fileSize: fileSize,        // ← AutoLoader.java dùng để so sánh trước khi xóa/tải lại
        message : upstreamData.message || 'OK'
      });
    }
