...
const MAX_BODY_BYTES = 512 * 1024; // 512 KB — đủ rộng cho mọi request hợp lệ
...
const HTML_LINES = [
  ...
  "      </button>`;",
  "    }",
  "    // Nút gia hạn từng key riêng (chỉ hiện với key có thời hạn)",
  "    if(k.hasExpiryPlan){",
  "      actionsHtml += `",
  ...
  "    $('extendKeyAmount').value = 30;",
  "    $('extendKeyUnit').value = 'day';",
  "    $('extendKeyMinutes').value = 0;",
  "    $('extendKeyPreview').textContent = '';",
  ...
  "}",
  "",
  "['extendKeyMinutes','extendKeyHours','extendKeyAmount','extendKeyMonths'].forEach(function(id){",
  "  var el = $id(id);",
  "  if(el) el.addEventListener('input', function(){",
  "    var mins   = Math.max(0, parseInt($id('extendKeyMinutes').value)||0);",
  "    var hours  = Math.max(0, parseInt($id('extendKeyHours').value)||0);",
  "    var days   = Math.max(0, parseInt($id('extendKeyAmount').value)||0);",
  "    var months = Math.max(0, parseInt($id('extendKeyMonths').value)||0);",
  "    $id('extendKeyPreview').textContent = calcExtendKeyPreview(mins,hours,days,months);",
  "  });",
  "});",
...
function readJSONBody(req){
  return new Promise((resolve, reject)=>{
    let data = '';
    let size = 0;
    // Slow-loris timeout: nếu sau SLOW_REQ_TIMEOUT_MS giây vẫn chưa nhận xong body → đóng kết nối
    ...
    req.on('data', chunk=>{
      size += chunk.length;
      // Payload size guard: từ chối body > MAX_BODY_BYTES (512 KB)
