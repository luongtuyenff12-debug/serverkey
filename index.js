...
// Upload snippet có thể lên tới 10 MB file text -> JSON encode thêm ~5% overhead -> để 15 MB.
// Endpoint thường (login, verify, state...) vẫn an toàn vì readJSONBody dùng giới hạn riêng.
const MAX_BODY_BYTES       = 15 * 1024 * 1024; // 15 MB - đủ cho upload 10 MB script
const MAX_BODY_BYTES_SMALL = 512 * 1024;        // 512 KB - cho mọi endpoint thường

/* ---------- 3) SLOW-LORIS TIMEOUT ---------- */
const SLOW_REQ_TIMEOUT_MS = 15 * 1000; // 15 giây để gửi xong toàn bộ request body
...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if((pathname === '/api/verify' || (pathname === '/' && url.searchParams.get('key'))) && (req.method === 'GET' || req.method === 'POST')){
    ...
    if(pathname === '/api/telegram/webhook' && req.method === 'POST'){
      try{
        ...
        if(msg && msg.from && typeof msg.text === 'string'){
          ...
          // /verify <ID> — xác thực ID qua Playwright headless
          } else if(textLower.startsWith('/verify')){
            const parts = text.split(/\s+/);
            const accountId = (parts[1] || '').trim();
            if(!accountId || !/^\d{6,15}$/.test(accountId)){
              await sendTelegramMessage(chatId,
                '❌ <b>Sai cú pháp.</b>\n\nDùng: <code>/verify &lt;ID_của_bạn&gt;</code>\n\nVí dụ: <code>/verify 15886913287</code>'
              );
            } else {
              // Thông báo đang xử lý
              await sendTelegramMessage(chatId,
                `⏳ <b>Đang xác thực ID:</b> <code>${accountId}</code>\n\nBot đang mở trình duyệt và tự động click... Vui lòng chờ 30-90 giây.`
              );
              // Lưu accountId vào map để auto-reverify
              _tgVerifySessions = _tgVerifySessions || {};
              _tgVerifySessions[String(chatId)] = { accountId, startedAt: Date.now() };
              // Chạy Playwright bất đồng bộ
              runPlaywrightVerify(chatId, accountId).catch(e=>{
                console.error('[TgVerify] Lỗi Playwright:', e && e.message);
          ...
          // /status — kiểm tra trạng thái phiên xác thực
          } else if(textLower === '/status'){
            _tgVerifySessions = _tgVerifySessions || {};
            const sess = _tgVerifySessions[String(chatId)];
            if(!sess){
              await sendTelegramMessage(chatId,
                'ℹ️ Chưa có phiên xác thực nào. Dùng <code>/verify &lt;ID&gt;</code> để bắt đầu.'
              );
            } else if(sess.expires && sess.expires > Date.now()){
              ...
              await sendTelegramMessage(chatId,
                `✅ <b>Đang xác thực</b>\nID: <code>${sess.accountId}</code>\n⏱ Còn lại: <b>${timeStr}</b>`
              );
            } else {
              await sendTelegramMessage(chatId,
                `ℹ️ Phiên xác thực đã hết hạn.\nID cũ: <code>${sess.accountId}</code>\nDùng <code>/verify ${sess.accountId}</code> để xác thực lại.`
              );
            }
          }
        }
...
/* ================================================================
   PLAYWRIGHT AUTO-VERIFY — Tự động click qua unlockffbeta.com
   ================================================================
   Luồng:
     1. User gửi /verify <ID>
     2. Bot mở Playwright headless → load unlockffbeta.com
     3. Điền accountId, click "Continue without Discord" × 4 bước
     4. Đọc timer "Access Granted"
     5. Gửi lại "✅ Xác thực xong, còn Xh Ym"
     6. Lưu expires vào _tgVerifySessions
     7. Auto-reverify khi còn <= 60 giây

   Cần: npm install playwright (thêm vào package.json)
   Trên Render Free: playwright-chromium tốn ít RAM hơn playwright đầy đủ.
================================================================ */

// Map lưu session của từng chat: { chatId -> { accountId, expires, reverifyTimer } }
let _tgVerifySessions = {};

// Lazy-load playwright để không crash nếu chưa cài
let _playwrightLib = null;
async function _getPlaywright(){
  if(_playwrightLib) return _playwrightLib;
  try{
    _playwrightLib = require('playwright');
    return _playwrightLib;
  }catch(e){
    // Thử require chromium riêng (playwright-chromium)
    try{
      _playwrightLib = require('playwright-chromium');
      return _playwrightLib;
    }catch(e2){
      throw new Error('Playwright chưa được cài. Chạy: npm install playwright');
    }
  }
}

/**
 * Chạy Playwright headless, tự động xác thực ID trên unlockffbeta.com,
 * gửi kết quả về chatId và lưu session + đặt timer auto-reverify.
 */
async function runPlaywrightVerify(chatId, accountId){
  const pw = await _getPlaywright();
  let browser = null;
  try{
    console.log(`[TgVerify] Bắt đầu verify chatId=${chatId} ID=${accountId}`);
    ...
    await context.route('**/*', (route)=>{
      ...
      const adPatterns = ['doubleclick','googlesyndication','adsterra','propellerads',
                          'popads','exoclick','popcash','onclickmega','clickadu','onesignal',
                          'pushcrew','taboola','outbrain','mgid','evadav'];
    ...
    // Timeout tổng: 120 giây
    page.setDefaultTimeout(120000);

    await page.goto('https://www.unlockffbeta.com/', { waitUntil: 'domcontentloaded', timeout: 30000 });

    // Helper: tìm nút "Continue without Discord" bằng text
    const findContinueBtn = async ()=>{
      return page.evaluate(()=>{
        const all = Array.from(document.querySelectorAll('button, a, [role="button"], [class*="btn"], [class*="continue"]'));
        for(const el of all){
          const txt = (el.innerText || el.textContent || '').toLowerCase().replace(/\s+/g,' ').trim();
          if(txt === 'continue without discord') return true;
          if(txt.includes('without') && txt.includes('discord') && !txt.includes('join') && txt.length < 120) return true;
        }
        return false;
      });
    };

    // Helper: điền ID và click nút
    const fillAndClick = async ()=>{
      // Điền ID vào input
      const input = await page.$('input[type="number"], input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"])');
      if(input){
        await input.fill('');
        await input.fill(accountId);
        await page.waitForTimeout(400);
      }
      // Click nút "Continue without Discord"
      const clicked = await page.evaluate((id)=>{
        ...
        for(const el of all){
          ...
          if((txt === 'continue without discord' || (txt.includes('without')&&txt.includes('discord')&&!txt.includes('join')&&txt.length<120)) && isClickable()){
            // Force enable + click
    ...
    // Helper: đọc timer từ trang "Access Granted"
    const readTimer = async ()=>{
      return page.evaluate(()=>{
        ...
        let m2;
        m2 = body.match(/(\d+)h\s*(\d+)m\s*(\d+)s/); if(m2) return +m2[1]*3600 + +m2[2]*60 + +m2[3];
        m2 = body.match(/(\d+)m\s*(\d+)s/);           if(m2) return +m2[1]*60 + +m2[2];
        return -1;
      });
    };
    ...
    const getStep = async ()=>{
      return page.evaluate(()=>{
        ...
        if(b.includes('access granted')) return 'granted';
        if(b.includes('unlocking')) return 'unlocking';
        if(b.includes('unlocked')&&b.includes('continue')) return 'unlocked';
        if(b.includes('without discord')||b.includes('verify')||b.includes('account')) return 'step1';
        return 'unknown';
      });
    };

    // ── MAIN LOOP: tối đa 90 giây, poll 1s/lần ──
    let success = false;
    let expiresSecs = 0;
    ...
    while(Date.now() < deadline){
      ...
      if(step === 'granted'){
        expiresSecs = await readTimer();
        if(expiresSecs > 0){
          success = true;
          break;
        }
        // Thử lại đọc timer sau 1s
        await page.waitForTimeout(1000);
        expiresSecs = await readTimer();
        if(expiresSecs > 0){ success = true; break; }
        // Vẫn không đọc được, đợi thêm
        continue;
      }

      if(step === 'unlocking'){
        // Đang unlock — chờ countdown rồi click Continue
        const clicked = await page.evaluate(()=>{
          const all = Array.from(document.querySelectorAll('button,[role="button"],[class*="btn"],[class*="continue"]'));
          for(const el of all){
            const t = (el.innerText||'').toLowerCase().trim();
            if(t.includes('continue')&&!t.includes('discord')&&!t.includes('join')){
              el.click(); return true;
            }
          }
          return false;
        });
        if(clicked) lastClickAt = Date.now();
        continue;
      }
      ...
      if(step === 'unlocked'){
        if(Date.now() - lastClickAt > 2000){
          await page.evaluate(()=>{
            ...
            for(const el of all){
              const t = (el.innerText||'').toLowerCase().trim();
              if(t.includes('continue')&&!t.includes('cancel')&&!t.includes('discord')&&!t.includes('restart')){
                el.click(); break;
              }
            }
          });
      ...
      // step1 hoặc unknown — điền ID và click
      if(Date.now() - lastClickAt > 1500){
        const clicked = await fillAndClick();
        if(clicked){ lastClickAt = Date.now(); clickCount++; }
        else {
          // Scroll nhẹ để kích hoạt JS
          await page.evaluate(()=>window.scrollTo(0, document.body.scrollHeight/2));
        }
      }
    }

    if(success && expiresSecs > 0){
      ...
      // Lưu session
      _tgVerifySessions = _tgVerifySessions || {};
      _tgVerifySessions[String(chatId)] = { accountId, expires: expiresAt };

      await sendTelegramMessage(chatId,
        `✅ <b>Xác thực xong!</b>\nID: <code>${accountId}</code>\n⏱ Còn lại: <b>${timeStr}</b>\n\nBot sẽ tự động xác thực lại khi còn dưới 60 giây.`
      );

      // Đặt timer auto-reverify khi còn 60s
      ...
      if(sess){
        sess.reverifyTimer = setTimeout(async ()=>{
          ...
          if(!cur || cur.accountId !== accountId) return; // session đã đổi
          if(cur.expires && cur.expires < Date.now() + 70000) return; // đã sắp hết thật
          await sendTelegramMessage(chatId,
            `🔄 <b>Tự động xác thực lại...</b>\nID: <code>${accountId}</code>`
          );
          runPlaywrightVerify(chatId, accountId).catch(e=>{
            console.error('[TgVerify] Auto-reverify lỗi:', e && e.message);
            sendTelegramMessage(chatId,
              `⚠️ <b>Tự động xác thực lại thất bại.</b>\nDùng <code>/verify ${accountId}</code> để thử lại.`
            );
          });
        }, reverifyIn);
      }

      console.log(`[TgVerify] OK chatId=${chatId} ID=${accountId} expires=${expiresSecs}s`);
    } else {
      await sendTelegramMessage(chatId,
        `❌ <b>Xác thực thất bại.</b>\nID: <code>${accountId}</code>\n\nKhông đọc được timer sau 90 giây. Có thể:\n• Trang unlockffbeta.com đang bận\n• ID không hợp lệ\n\nThử lại: <code>/verify ${accountId}</code>`
      );
      console.warn(`[TgVerify] TIMEOUT chatId=${chatId} ID=${accountId}`);
    }
  }catch(e){
    console.error('[TgVerify] Lỗi:', e && e.message);
    await sendTelegramMessage(chatId,
...
async function startTelegramBot(){
  ...
  await telegramApiCall('setMyCommands', {
    commands: [
      { command: 'start',  description: 'Mở Shop mua key' },
      { command: 'verify', description: 'Xác thực ID — ví dụ: /verify 15886913287' },
      { command: 'status', description: 'Kiểm tra trạng thái xác thực hiện tại' }
    ]
  });
}
