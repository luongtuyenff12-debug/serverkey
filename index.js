...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    /* =====================================================================
       ...
       GET/POST /api/loadlib/check
       Trả về URL tải thư viện, SHA256 xác thực, phiên bản hiện hành.
       ===================================================================== */
    if(pathname === '/api/loadlib/check' && (req.method === 'GET' || req.method === 'POST')){
      const loadlibIp = getClientIP(req);
      // Giới hạn tốc độ: 30 lần/phút/IP — đủ cho app hợp lệ, chặn brute-force
      if(isRateLimited('loadlib_check', loadlibIp, 30, 60 * 1000)){
      ...
      // Hằng số cấu hình (lấy từ text.txt)
      ...
      // Kiểm tra User-Agent: chỉ chấp nhận đúng app CakModsLoader
      if(userAgent && !userAgent.includes(LOADLIB_CONFIG.HTTP_USER_AGENT)){
      ...
      // Thành công — trả về đúng cấu trúc [A]
      console.log(`[LoadLib] OK — ${loadlibIp} => ${upstreamData.file_url} v${upstreamData.version || 'N/A'}`);
      return sendJSON(res, 200, {
        status  : 'SUCCESS',
        file_url: upstreamData.file_url,
