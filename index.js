...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if((pathname === '/api/loadlib/check' || pathname === '/api/loadlib/check-cakmods')
      ...
      // Gọi check.php upstream để lấy version/sha256 mới nhất tự động
      // check.php trả JSON: {"status":"success","version":"...","file_url":"...","sha256":"..."}
      ...
      try {
        upstreamData = await new Promise((resolve, reject) => {
          ...
          const upReq = proto.request({
            ...
            upRes.on('end', () => {
              if(upRes.statusCode !== 200) return reject(new Error(`upstream_http_${upRes.statusCode}`));
              // check.php có thể trả JSON hoặc plain text tuỳ param
              try {
                const parsed = JSON.parse(raw);
                resolve(parsed);
              } catch(e) {
                // Nếu plain text (ví dụ chỉ trả version number), tự build object
                const ver = raw.trim();
                if(ver && /^[\d.]+$/.test(ver)){
                  const builtFileUrl = LOADLIB_CONFIG.BASE_URL + 'libCakMods_' + ver + '.0.so';
                  resolve({ status: 'success', version: ver + '.0', file_url: builtFileUrl, sha256: '' });
                } else {
                  reject(new Error('upstream_invalid_response'));
                }
              }
            });
          });
        ...
        console.warn(`[LoadLib] Upstream check.php lỗi (${upErr && upErr.message}), dùng cache cuối.`);
      }

      // Nếu upstream lỗi hoàn toàn, dùng cache gần nhất (lưu trong RAM)
      if(!upstreamData || !String(upstreamData.status || '').toLowerCase().startsWith('s')){
        if(global._loadlibCache){
          console.warn(`[LoadLib] Dùng cache: ${JSON.stringify(global._loadlibCache)}`);
          upstreamData = global._loadlibCache;
        } else {
          return sendJSON(res, 502, {
            status : 'ERROR',
            error  : 'Không thể kết nối server thư viện và chưa có cache.',
            message: 'Server returned error: 502'
          });
        }
      }

      // Lưu cache RAM để dùng khi upstream tạm thời lỗi
      global._loadlibCache = upstreamData;

      // Lấy fileUrl từ upstream (hỗ trợ cả file_url lẫn fileUrl)
      const fileUrl = upstreamData.file_url || upstreamData.fileUrl
        || (LOADLIB_CONFIG.BASE_URL + 'libCakMods_' + (upstreamData.version || '') + '.so');

      // Lấy fileSize bằng HEAD request
      let fileSize = upstreamData.fileSize || upstreamData.file_size || 0;
      if(!fileSize && fileUrl){
        try {
          fileSize = await new Promise((resolve) => {
            const fUrl = new URL(fileUrl);
