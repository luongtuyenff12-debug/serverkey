...
const HTML_LINES = [
  ...
  "  try{",
  "    await fetch(API_BASE + '/api/state/keys', { method:'POST', headers:{'Content-Type':'application/json'}, body: snap });",
  "  }catch(e){",
  "    // fallback: nếu endpoint riêng chưa có thì dùng /api/state với flag mergeKeys",
  "    try{",
  "      await fetch(API_BASE + '/api/state', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ keysStore, _mergeDevices: true }) });",
  "    }catch(e2){",
  "      console.warn('[KeyVault] Lưu keysStore thất bại:', e2);",
  "    }",
  "  }",
  "}",
  "",
...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if(pathname === '/api/verify' && (req.method === 'GET' || req.method === 'POST')){
      ...
      // saveDBDebounced() delay 150ms → nếu dashboard load /api/state ngay sau verify,
      // dữ liệu thiết bị chưa được ghi → hiện 0/1. saveDBNow() ghi đồng bộ tức thì.
      saveDBNow();
      return sendJSON(res, 200, {
        valid,
        status,
